...
export default function WorkSalon({ closeOnOverlayClick }) {
    ...
    const [selectId, setSelectId] = React.useState(selectModalId)
    ...
    const getWorks = async (id) => {
        const res = await worksApi.getWork({ id: id })
        return res
    }
    const handleArrowClick = async (isNext) => {
        if (selectId !== -1 ) {
            ...
            const res = await getWorks(
                slidedataIds[
                    !isNext && selectId !== 0 ? selectId - 1 : selectId + 1
                ]
            )
            if (res) {
                setSelectId((prev) =>
